export function thickenEdgeLines() {
  ...
  const elementsToClear = document.querySelectorAll(
    "#board > .element:not(.static-element)"
  );
  ...
  const verticalRowElements = Array.from(
    document.querySelectorAll("#board > .thin-vertical")
  );
  ...
  const horizontalLineElements = Array.from(
    document.querySelectorAll("#board > .thin-horizontal")
  );
  ...
  obstacles.forEach((obstacle) => {
    ...
    const squareNeighbors = neighbors.filter((pos) => {
      if (pos.row >= 0 && pos.row < 33 && pos.col >= 0 && pos.col < 33) {
        ...
        return (
          neighborElement.classList.contains("square") &&
          !neighborElement.classList.contains("static-element")
        );
      }
      return false;
    });
  ...
  window.selectedSquares = Array.from(
    document.querySelectorAll("#board > .selectedSquare")
  );
}
...
export function thickenCorners() {
  ...
  quadrants.forEach((quad) => {
    ...
    while (selectedSquares.length < 4) {
      ...
      if (
        square.classList.contains("selectedSquare") ||
        square.classList.contains("corner")
      ) {
      ...
      const conflict = neighbors.some((pos) => {
        if (pos.row >= 0 && pos.row < 16 && pos.col >= 0 && pos.col < 16) {
          ...
          if (
            neighborSquare.classList.contains("selectedSquare") ||
            neighborSquare.classList.contains("corner")
          ) {
  ...
  newQuadrants.forEach((quad, quadrantIndex) => {
    ...
    if (quadrantCorners.length !== 4) {
      console.error(
        `Error in quadrant ${quadrantIndex}: Expected 4 corners, found ${quadrantCorners.length}`
      );
      return;
    }
    ...
    const shuffledConfigurations = configurations.sort(
      () => Math.random() - 0.5
    );
  ...
  window.cornerSquares = Array.from(
    document.querySelectorAll("#board > .corner")
  );
}
