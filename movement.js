...
import { resetButton, undoButton } from "./eventListeners.js";
...
export function clearHighlightedSquares() {
  const highlightedSquares = document.querySelectorAll(
    "#board > .element.square.highlighted"
  );
  ...
  highlightedSquares.forEach((square) => {
    if (!square.classList.contains("colored")) {
      // Only clear non-colored highlighted squares
...
function saveCurrentState() {
  const currentState = {
    coloredSquares: Array.from(
      document.querySelectorAll("#board > .element.square.colored")
    ).map((square) => ({
      element: square, // Reference to the square
      backgroundColor: square.style.backgroundColor, // Store its current color
    })),
    lastClickedIndex: originalColoredSquare
      ? Array.from(document.querySelectorAll("#board > .element")).indexOf(
          originalColoredSquare
        )
      : -1, // Save index of last clicked square
  };
...
export function undoLastMove() {
  ...
  const lastClickedSquare =
    lastState.lastClickedIndex !== -1
      ? elements[lastState.lastClickedIndex]
      : null;
...
export function handleColoredSquareClick(event) {
  ...
  for (let r = row - 1; r >= 0; r--) {
    ...
    if (square.classList.contains("square")) {
      // Check if it has the 'square' class
...
export function handleHighlightedSquareClick(event) {
  ...
  const currentArmClass = armClasses.find((armClass) =>
    clickedSquare.classList.contains(armClass)
  );

  // Get all highlighted squares
  const highlightedSquares = Array.from(
    document.querySelectorAll("#board > .element")
  ); // Select 33x33 grid

  // Filter for squares with the correct arm class
  const squaresInDirection = highlightedSquares.filter((square) =>
    square.classList.contains(currentArmClass)
  );
  ...
  if (currentArmClass === "armUp" || currentArmClass === "armLeft") {
    ...
    targetSquare = squaresInDirection.reduce((min, sq) =>
      highlightedSquares.indexOf(sq) < highlightedSquares.indexOf(min)
        ? sq
        : min
    );
    ...
    targetSquare = squaresInDirection.reduce((max, sq) =>
      highlightedSquares.indexOf(sq) > highlightedSquares.indexOf(max)
        ? sq
        : max
    );
  }
  ...
  const circleColor = circleElement
    ? window.getComputedStyle(circleElement).backgroundColor
    : null;
  ...
  if (targetSquare) {
    ...
    setTimeout(() => {
      ...
      const shouldRotate =
        targetSquare.classList.contains("target") &&
        originalColor === circleColor;
